...
parallel_symbol=zeros(N_carrier,N_symbol);
    ...
    for n=1:N_symbol % OQAM preprocessing
    ...
    for n=1:N_symbol
        ...
        FBMC_serial(1+(n-1)*N_fft/2:(n-1)*N_fft/2+K*N_fft)=
        FBMC_serial(1+(n-1)*N_fft/2:(n-1)*N_fft/2+K*N_fft)+filtered_output;
    end
 for i=1:length(snr_db)
     ...
     for n=1:N_symbol
        R=received(1+(n-1)*N_fft/2:(n-1)*N_fft/2+K*N_fft).*h; % de-filering
        U=zeros(1,N_fft);
         for k=1:K
            U=U+R(1,1+(k-1)*N_fft:k*N_fft); % extraction                
         end
        U=U.';
        SEST=fft(U);
        ...
        SEST=[SEST_1;SEST_2];       
        if rem(n,2)==1 % OQAM postprocessing
        ...
        bit_demod=qamdemod(receiver_demod,M,'OutputType','bit')';
...
figure;
semilogy(ebn0_db,ber_fbmc_4,'k-o','linewidth',1.5,'color','b');
hold on;
semilogy(ebn0_db,ber_thertical_4,'k-o','linewidth',1.5,'color','r');
hold on;
% semilogy(ebn0_db,ber_fbmc_8,'k-o','linewidth',1.5,'color','k');
% hold on;
...
% figure(2)
% plot(parallel_symbol_before_oqam,'or');
% hold on
% plot(receiver_demod,'xk');
